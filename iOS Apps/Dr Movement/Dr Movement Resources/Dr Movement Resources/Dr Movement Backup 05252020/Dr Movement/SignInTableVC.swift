        ...
        func fireErrorHandle(error: Error) {
            ...
            if let errorCode = AuthErrorCode(rawValue: error._code){
                ...
                case .errorCodeEmailAlreadyInUse:
                    ...
                    showAlert(withTitle: "Email is already in use.", message: "")
                case .errorCodeInvalidEmail:
                    print("Indicates the email is invalid")
                    showAlert(withTitle: "Email is invalid.", message: "")
                case .errorCodeWrongPassword:
                    print("Indicates the user attempted sign in with a wrong password")
                    showAlert(withTitle: "That password is incorrect!", message: "")
                case .errorCodeTooManyRequests:
                    print("Indicates that too many requests were made to a server method")
                    showAlert(withTitle: "Too many failed attempts.", message: "")
                case .errorCodeUserNotFound:
                    print("Indicates the user account was not found")
                    showAlert(withTitle: "User account not found", message: "")
...
extension SignInTableVC: UITextFieldDelegate {
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        // deletes current text in textField
        //self.mainTableView.setContentOffset(CGPoint(x: 0, y: 100), animated: true)
    }
